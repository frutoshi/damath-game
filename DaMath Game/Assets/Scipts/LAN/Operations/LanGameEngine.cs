...
public class LanGameEngine : NetworkBehaviour
{
    ...
    [ClientRpc]
    public void RpcSetIsFull(bool stat) 
    {
        Debug.Log("RpcIsFull Executed");
        isFull = true;
    ...
    [ClientRpc]
    public void RpctestMove()
    {
        pieces[0, 1].transform.position = new Vector3(0, 0, 0);
    }
    ...
    void Update()
    {
        if (goPlayer1 != null) 
        {
            if (goPlayer1.GetComponent<PlayerLanScript>().isLocalPlayer && Input.GetKeyDown(KeyCode.P))
            {
                Debug.Log("Start Game");
                startGame();
            }
            if (goPlayer1.GetComponent<PlayerLanScript>().isLocalPlayer && Input.GetKeyDown(KeyCode.O))
            {
                RpctestMove();
            }
        }
